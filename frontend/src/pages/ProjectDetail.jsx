  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-4">
        <Link to="/projects" className="text-gray-400 hover:text-white"><ArrowLeft size={20} /></Link>
        <div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 bg-navy-light border-b border-white/10">
          <span className="font-semibold text-white">Contratistas Asignados</span>
          <button onClick={() => setShowAssign(true)} className="btn-primary btn-sm flex items-center gap-1">
            <Plus size={14} /> Asignar
          </button>
        </div>
        {contractors.length === 0 ? (
          <p className="px-5 py-6 text-gray-400 text-sm">No hay contratistas asignados.</p>
        ) : (
            ...
            <tbody>
              {contractors.map(c => (
                <tr key={c.contractor_id} className="table-row text-sm">
                  <td className="px-5 py-3 text-white font-medium">{c.contractor_name}</td>
                  <td className="px-5 py-3 text-right font-mono text-gray-300">{mxn(c.valor_presupuesto)}</td>
      ...
      {history.length > 0 && (
        <div className="glass-card overflow-hidden">
          <div className="px-5 py-3 bg-navy-light border-b border-white/10">
            <span className="font-semibold text-white">Historial Semanal</span>
          </div>
          <table className="w-full">
            <thead>
              <tr className="text-xs text-gray-400 uppercase tracking-wider border-b border-white/10">
            ...
            <tbody>
              {history.map((h, i) => (
                <tr key={i} className="table-row text-sm">
                  ...
                  <td className="px-5 py-3 text-right font-mono text-gray-300">{mxn(h.total_ent)}</td>
                  <td className="px-5 py-3 text-right font-mono text-green-400 font-semibold">{mxn(h.total_rep)}</td>
                  <td className="px-5 py-3 text-right text-gray-400">{h.contractors_active}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showAssign && (
        <Modal title="Asignar Contratista" onClose={() => setShowAssign(false)} size="sm">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">Valor Presupuesto (VP)</label>
      ...
      {editBudget && (
        <Modal title={`Editar VP — ${editBudget.contractor_name}`} onClose={() => setEditBudget(null)} size="sm">
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Nuevo V.P.</label>
              <input type="number" className="input-field" value={budgetVal}
                onChange={e => setBudgetVal(Number(e.target.value))} />
              <p className="text-xs text-gray-500 mt-1">El cambio aplica desde esta semana en adelante. El historial no se modifica.</p>
            </div>
            <div className="flex gap-3 justify-end">
